...
namespace WebApplication1.Controllers
{
    public class CustomerController : Controller
    {
        ...
        public IActionResult Increase(int selectId, int quantity, int price)
        {
            ...
            return Json(new { producttotalprice = s.TotalPrice, totalprice = selectedItemsRepo.getall().Sum(s => s.TotalPrice), totalquantity = selectedItemsRepo.getall().Where(s => s.CustomerID == customer.ID).Sum(s => s.Quantity) });
        }
